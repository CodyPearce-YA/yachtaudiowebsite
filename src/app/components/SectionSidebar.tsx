...
export function SectionSidebar({ items, progress, onNavigate }: Props) {
  ...
  return (
    <div className="fixed right-8 top-1/2 z-40 pointer-events-none" style={{ transform: 'translateY(-50%)' }}>
      <div className="pointer-events-auto flex flex-col gap-5 items-end opacity-25 hover:opacity-100 transition-opacity duration-500" onMouseLeave={() => setHovered(null)}>
        {items.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => onNavigate(item.id)} onMouseEnter={() => setHovered(item.id)} className="flex items-center gap-3 group">
              ...
              <div style={{ width: active ? 8 : isHovered ? 6 : 4, height: active ? 8 : isHovered ? 6 : 4, borderRadius: '50%', backgroundColor: active ? '#A2834E' : '#d1d5db', boxShadow: active ? '0 0 8px rgba(162,131,78,0.55)' : 'none', flexShrink: 0, transition: 'all 0.35s cubic-bezier(0.16,1,0.3,1)' }} />
            </button>
          )
        })}
      </div>
